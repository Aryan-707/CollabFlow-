...
// MCP Server configuration for CollabFlow context
export interface CollabFlowContext {
...
export interface MCPTool {
  ...
  description: string;
  parameters?: any;
  execute: (params: any, context: CollabFlowContext) => Promise<any>;
}

class MCPServer {
  private context: CollabFlowContext = {};
  private tools: Map<string, MCPTool> = new Map();
  private conversationHistory: ChatMessage[] = [];
  private sessionId: string = this.getOrCreateSessionId();

  // Initialize MCP server with context
  initialize(context: CollabFlowContext = {}) {
  ...
  updateContext(updates: Partial<CollabFlowContext>) {
...
export function extractContextFromPath(pathname: string): Partial<CollabFlowContext> {
  const context: Partial<CollabFlowContext> = {};
